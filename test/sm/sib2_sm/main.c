...
#include "../../rnd/fill_rnd_data_sib2.h"
#include "../../../src/sm/sib2_sm/sib2_sm_agent.h"
#include "../../../src/sm/sib2_sm/sib2_sm_ric.h"
...
static
bool read_sib2_ind(void* read)
{
  assert(read != NULL);

  sib2_ind_data_t* sib2 = (sib2_ind_data_t*)read;

  fill_sib2_ind_data(sib2);
  cp.hdr = cp_sib2_ind_hdr(&sib2->hdr);
  cp.msg = cp_sib2_ind_msg(&sib2->msg);
  return true;
}
...
void check_indication(sm_agent_t* ag, sm_ric_t* ric)
{
 ...
 sm_ag_if_rd_ind_t msg = ric->proc.on_indication(ric, &exp.data);
  ...
  assert(msg.type == SIB2_STATS_V0);

  sib2_ind_data_t* data = &msg.sib2;

 if(msg.sib2.msg.rb != NULL){
      assert(msg.sib2.msg.len != 0);
 } 

  assert(eq_sib2_ind_hdr(&data->hdr, &cp.hdr) == true);
  assert(eq_sib2_ind_msg(&data->msg, &cp.msg) == true);
  assert(eq_sib2_call_proc_id(data->proc_id, cp.proc_id) == true);

  free_sib2_ind_hdr(&data->hdr);
  free_sib2_ind_msg(&data->msg);
...
int main()
{
  ...
  io_ag.read_ind_tbl[SIB2_SUBS_V0] = read_sib2_ind; 

  sm_agent_t* sm_ag = make_sib2_sm_agent(io_ag);

  sm_ric_t* sm_ric = make_sib2_sm_ric();
