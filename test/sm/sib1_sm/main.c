...
#include "../../rnd/fill_rnd_data_sib1.h"
#include "../../../src/sm/sib1_sm/sib1_sm_agent.h"
#include "../../../src/sm/sib1_sm/sib1_sm_ric.h"
...
static
bool read_sib1_ind(void* read)
{
  assert(read != NULL);

  sib1_ind_data_t* sib1 = (sib1_ind_data_t*)read;

  fill_sib1_ind_data(sib1);
  cp.hdr = cp_sib1_ind_hdr(&sib1->hdr);
  cp.msg = cp_sib1_ind_msg(&sib1->msg);
  return true;
}
...
void check_indication(sm_agent_t* ag, sm_ric_t* ric)
{
 ...
 sm_ag_if_rd_ind_t msg = ric->proc.on_indication(ric, &exp.data);
  ...
  assert(msg.type == SIB1_STATS_V0);

  sib1_ind_data_t* data = &msg.sib1;

 if(msg.sib1.msg.rb != NULL){
      assert(msg.sib1.msg.len != 0);
 } 

  assert(eq_sib1_ind_hdr(&data->hdr, &cp.hdr) == true);
  assert(eq_sib1_ind_msg(&data->msg, &cp.msg) == true);
  assert(eq_sib1_call_proc_id(data->proc_id, cp.proc_id) == true);

  free_sib1_ind_hdr(&data->hdr);
  free_sib1_ind_msg(&data->msg);
...
int main()
{
  ...
  io_ag.read_ind_tbl[SIB1_SUBS_V0] = read_sib1_ind; 

  sm_agent_t* sm_ag = make_sib1_sm_agent(io_ag);

  sm_ric_t* sm_ric = make_sib1_sm_ric();
