...
#include "../../rnd/fill_rnd_data_rr.h"
#include "../../../src/sm/rr_sm/rr_sm_agent.h"
#include "../../../src/sm/rr_sm/rr_sm_ric.h"
...
static
bool read_rr_ind(void* read)
{
  assert(read != NULL);

  rr_ind_data_t* rr = (rr_ind_data_t*)read;

  fill_rr_ind_data(rr);
  cp.hdr = cp_rr_ind_hdr(&rr->hdr);
  cp.msg = cp_rr_ind_msg(&rr->msg);
  return true;
}
...
void check_indication(sm_agent_t* ag, sm_ric_t* ric)
{
 ...
 sm_ag_if_rd_ind_t msg = ric->proc.on_indication(ric, &exp.data);
  ...
  assert(msg.type == RR_STATS_V0);

  rr_ind_data_t* data = &msg.rr;

 if(msg.rr.msg.rb != NULL){
      assert(msg.rr.msg.len != 0);
 } 

  assert(eq_rr_ind_hdr(&data->hdr, &cp.hdr) == true);
  assert(eq_rr_ind_msg(&data->msg, &cp.msg) == true);
  assert(eq_rr_call_proc_id(data->proc_id, cp.proc_id) == true);

  free_rr_ind_hdr(&data->hdr);
  free_rr_ind_msg(&data->msg);
...
int main()
{
  ...
  io_ag.read_ind_tbl[RR_SUBS_V0] = read_rr_ind; 

  sm_agent_t* sm_ag = make_rr_sm_agent(io_ag);

  sm_ric_t* sm_ric = make_rr_sm_ric();
