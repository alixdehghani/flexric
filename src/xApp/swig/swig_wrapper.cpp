...
static
void sm_cb_zxc(sm_ag_if_rd_t const* rd)
{
  assert(rd != NULL);
  assert(rd->type == INDICATION_MSG_AGENT_IF_ANS_V0);
  assert(rd->ind.type == ZXC_STATS_V0);
  assert(hndlr_zxc_cb != NULL);

  zxc_ind_data_t const* data = &rd->ind.zxc; 

  swig_zxc_ind_msg_t ind;
  ind.tstamp = data->msg.tstamp;

  for(uint32_t i = 0; i < data->msg.len; ++i){
    zxc_radio_bearer_stats_t tmp = data->msg.rb[i];
    ind.rb_stats.push_back(tmp);
  }
...
int report_zxc_sm(global_e2_node_id_t* id, Interval inter_arg, zxc_cb* handler)
{

  assert(id != NULL);
  assert(handler != NULL);

  hndlr_zxc_cb = handler;

  const char* period = convert_period(inter_arg);

  sm_ans_xapp_t ans = report_sm_xapp_api(id, SM_ZXC_ID, (void*)period, sm_cb_zxc);
  assert(ans.success == true); 
  return ans.u.handle;
}
...
//static
//pthread_t t_enb_conf;

static 
enb_conf_cb* hndlr_enb_conf_cb; 

static
void sm_cb_enb_conf(sm_ag_if_rd_t const* rd)
{
  assert(rd != NULL);
  assert(rd->type == INDICATION_MSG_AGENT_IF_ANS_V0);
  assert(rd->ind.type == ENB_CONF_STATS_V0);
  assert(hndlr_enb_conf_cb != NULL);

  enb_conf_ind_data_t const* data = &rd->ind.enb_conf; 

  swig_enb_conf_ind_msg_t ind;
  ind.tstamp = data->msg.tstamp;

  for(uint32_t i = 0; i < data->msg.len; ++i){
    enb_conf_stats_t tmp = data->msg.rb[i];
    ind.rb_stats.push_back(tmp);
  }
...
int report_enb_conf_sm(global_e2_node_id_t* id, Interval inter_arg, enb_conf_cb* handler)
{

  assert(id != NULL);
  assert(handler != NULL);

  hndlr_enb_conf_cb = handler;

  const char* period = convert_period(inter_arg);

  sm_ans_xapp_t ans = report_sm_xapp_api(id, SM_ENB_CONF_ID, (void*)period, sm_cb_enb_conf);
  assert(ans.success == true); 
  return ans.u.handle;
}

void rm_report_enb_conf_sm(int handler)
{
