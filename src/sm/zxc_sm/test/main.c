...
#include "../../zxc_sm/zxc_sm_agent.h"
#include "../../zxc_sm/zxc_sm_ric.h"
...
{
  assert(read != NULL);
  assert(read->type == ZXC_STATS_V0);

  fill_zxc_ind_data(&read->zxc_stats);
  cp.hdr = cp_zxc_ind_hdr(&read->zxc_stats.hdr);
  cp.msg = cp_zxc_ind_msg(&read->zxc_stats.msg);
}
...
void check_indication(sm_agent_t* ag, sm_ric_t* ric)
{
 ...
 sm_ag_if_rd_t msg = ric->proc.on_indication(ric, &sm_data);
  ...
  zxc_ind_data_t* data = &msg.zxc_stats;

 if(msg.zxc_stats.msg.rb != NULL){
      assert(msg.zxc_stats.msg.len != 0);
 } 
