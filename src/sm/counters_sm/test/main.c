...
#include "../../counters_sm/counters_sm_agent.h"
#include "../../counters_sm/counters_sm_ric.h"
...
{
  assert(read != NULL);
  assert(read->type == COUNTERS_STATS_V0);

  fill_counters_ind_data(&read->counters_stats);
  cp.hdr = cp_counters_ind_hdr(&read->counters_stats.hdr);
  cp.msg = cp_counters_ind_msg(&read->counters_stats.msg);
}
...
void check_indication(sm_agent_t* ag, sm_ric_t* ric)
{
 ...
 sm_ag_if_rd_t msg = ric->proc.on_indication(ric, &sm_data);
  ...
  counters_ind_data_t* data = &msg.counters_stats;

 if(msg.counters_stats.msg.rb != NULL){
      assert(msg.counters_stats.msg.len != 0);
 } 
