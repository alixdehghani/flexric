...
byte_array_t rr_enc_event_trigger_plain(rr_event_trigger_t const* event_trigger)
{
...
byte_array_t rr_enc_action_def_plain(rr_action_def_t const* action_def)
{
...
byte_array_t rr_enc_ind_hdr_plain(rr_ind_hdr_t const* ind_hdr)
{
  assert(ind_hdr != NULL);

  byte_array_t ba = {0};

  ba.len = sizeof(rr_ind_hdr_t);
...
byte_array_t rr_enc_ind_msg_plain(rr_ind_msg_t const* ind_msg)
{
  assert(ind_msg != NULL);
...
byte_array_t rr_enc_call_proc_id_plain(rr_call_proc_id_t const* call_proc_id)
{
  assert(0!=0 && "Not implemented");
...
byte_array_t rr_enc_ctrl_hdr_plain(rr_ctrl_hdr_t const* ctrl_hdr)
{
  assert(ctrl_hdr != NULL);
  byte_array_t  ba = {0};
  ba.len = sizeof(rr_ctrl_hdr_t);
  ba.buf = calloc(ba.len, sizeof(uint8_t)); 
  assert(ba.buf != NULL && "Memory exhausted");
...
byte_array_t rr_enc_ctrl_msg_plain(rr_ctrl_msg_t const* ctrl_msg)
{
  assert(ctrl_msg != NULL);
  size_t const sz = sizeof(ctrl_msg->len) + 
                  sizeof(rr_stats_t)*ctrl_msg->len;
  ...
  for(uint32_t i = 0; i < ctrl_msg->len ; ++i){
    memcpy(it, &ctrl_msg->rr_rb[i], sizeof(ctrl_msg->rr_rb[i]));
    it += sizeof(ctrl_msg->rr_rb[i]);
  }
...
byte_array_t rr_enc_ctrl_out_plain(rr_ctrl_out_t const* ctrl) 
{
  assert(0!=0 && "Not implemented");
...
byte_array_t rr_enc_func_def_plain(rr_func_def_t const* func)
{
  assert(0!=0 && "Not implemented");
