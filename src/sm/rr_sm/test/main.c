...
#include "../../rr_sm/rr_sm_agent.h"
#include "../../rr_sm/rr_sm_ric.h"
...
{
  assert(read != NULL);
  assert(read->type == RR_STATS_V0);

  fill_rr_ind_data(&read->rr_stats);
  cp.hdr = cp_rr_ind_hdr(&read->rr_stats.hdr);
  cp.msg = cp_rr_ind_msg(&read->rr_stats.msg);
}
...
void check_indication(sm_agent_t* ag, sm_ric_t* ric)
{
 ...
 sm_ag_if_rd_t msg = ric->proc.on_indication(ric, &sm_data);
  ...
  rr_ind_data_t* data = &msg.rr_stats;

 if(msg.rr_stats.msg.rb != NULL){
      assert(msg.rr_stats.msg.len != 0);
 } 
