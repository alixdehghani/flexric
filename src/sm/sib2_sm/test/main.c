...
#include "../../sib2_sm/sib2_sm_agent.h"
#include "../../sib2_sm/sib2_sm_ric.h"
...
{
  assert(read != NULL);
  assert(read->type == SIB2_STATS_V0);

  fill_sib2_ind_data(&read->sib2_stats);
  cp.hdr = cp_sib2_ind_hdr(&read->sib2_stats.hdr);
  cp.msg = cp_sib2_ind_msg(&read->sib2_stats.msg);
}
...
void check_indication(sm_agent_t* ag, sm_ric_t* ric)
{
 ...
 sm_ag_if_rd_t msg = ric->proc.on_indication(ric, &sm_data);
  ...
  sib2_ind_data_t* data = &msg.sib2_stats;

 if(msg.sib2_stats.msg.rb != NULL){
      assert(msg.sib2_stats.msg.len != 0);
 } 
