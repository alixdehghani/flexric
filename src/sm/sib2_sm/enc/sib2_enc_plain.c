...
byte_array_t sib2_enc_action_def_plain(sib2_action_def_t const* action_def)
{
...
byte_array_t sib2_enc_ind_hdr_plain(sib2_ind_hdr_t const* ind_hdr)
{
  assert(ind_hdr != NULL);

  byte_array_t ba = {0};

  ba.len = sizeof(sib2_ind_hdr_t);
...
byte_array_t sib2_enc_ind_msg_plain(sib2_ind_msg_t const* ind_msg)
{
  assert(ind_msg != NULL);
  ...
  size_t const sz = sizeof(ind_msg->len) + 
                  sizeof(sib2_stats_t)*ind_msg->len + 
...
byte_array_t sib2_enc_call_proc_id_plain(sib2_call_proc_id_t const* call_proc_id)
{
  assert(0!=0 && "Not implemented");
...
byte_array_t sib2_enc_ctrl_hdr_plain(sib2_ctrl_hdr_t const* ctrl_hdr)
{
  assert(ctrl_hdr != NULL);
  byte_array_t  ba = {0};
  ba.len = sizeof(sib2_ctrl_hdr_t);
  ba.buf = calloc(ba.len, sizeof(uint8_t)); 
  assert(ba.buf != NULL && "Memory exhausted");
...
byte_array_t sib2_enc_ctrl_msg_plain(sib2_ctrl_msg_t const* ctrl_msg)
{
...
byte_array_t sib2_enc_ctrl_out_plain(sib2_ctrl_out_t const* ctrl) 
{
  assert(0!=0 && "Not implemented");
...
byte_array_t sib2_enc_func_def_plain(sib2_func_def_t const* func)
{
  assert(0!=0 && "Not implemented");
