...
byte_array_t sib1_enc_event_trigger_plain(sib1_event_trigger_t const* event_trigger)
{
...
byte_array_t sib1_enc_action_def_plain(sib1_action_def_t const* action_def)
{
...
byte_array_t sib1_enc_ind_hdr_plain(sib1_ind_hdr_t const* ind_hdr)
{
  assert(ind_hdr != NULL);

  byte_array_t ba = {0};

  ba.len = sizeof(sib1_ind_hdr_t);
...
byte_array_t sib1_enc_ind_msg_plain(sib1_ind_msg_t const* ind_msg)
{
  assert(ind_msg != NULL);
  ...
  size_t const sz = sizeof(ind_msg->len) + 
                  sizeof(sib1_stats_t)*ind_msg->len + 
...
byte_array_t sib1_enc_call_proc_id_plain(sib1_call_proc_id_t const* call_proc_id)
{
  assert(0!=0 && "Not implemented");
...
byte_array_t sib1_enc_ctrl_hdr_plain(sib1_ctrl_hdr_t const* ctrl_hdr)
{
  assert(ctrl_hdr != NULL);
  byte_array_t  ba = {0};
  ba.len = sizeof(sib1_ctrl_hdr_t);
  ba.buf = calloc(ba.len, sizeof(uint8_t)); 
  assert(ba.buf != NULL && "Memory exhausted");
...
byte_array_t sib1_enc_ctrl_msg_plain(sib1_ctrl_msg_t const* ctrl_msg)
{
  assert(ctrl_msg != NULL);
  byte_array_t  ba = {0};
  size_t const sz = sizeof(ctrl_msg->len) + 
                  sizeof(sib1_stats_t)*ctrl_msg->len;
  // ba.len = sizeof(sib1_ctrl_hdr_t);
  ...
  for(uint32_t i = 0; i < ctrl_msg->len ; ++i){
    memcpy(it, &ctrl_msg->sib1_rb[i], sizeof(ctrl_msg->sib1_rb[i]));
    it += sizeof(ctrl_msg->sib1_rb[i]);
  }
  assert(it == ba.buf + sz && "Mismatch of data layout");
...
byte_array_t sib1_enc_ctrl_out_plain(sib1_ctrl_out_t const* ctrl) 
{
  assert(0!=0 && "Not implemented");
...
byte_array_t sib1_enc_func_def_plain(sib1_func_def_t const* func)
{
  assert(0!=0 && "Not implemented");
