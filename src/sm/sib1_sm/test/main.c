...
#include "../../sib1_sm/sib1_sm_agent.h"
#include "../../sib1_sm/sib1_sm_ric.h"
...
{
  assert(read != NULL);
  assert(read->type == SIB1_STATS_V0);

  fill_sib1_ind_data(&read->sib1_stats);
  cp.hdr = cp_sib1_ind_hdr(&read->sib1_stats.hdr);
  cp.msg = cp_sib1_ind_msg(&read->sib1_stats.msg);
}
...
void check_indication(sm_agent_t* ag, sm_ric_t* ric)
{
 ...
 sm_ag_if_rd_t msg = ric->proc.on_indication(ric, &sm_data);
  ...
  sib1_ind_data_t* data = &msg.sib1_stats;

 if(msg.sib1_stats.msg.rb != NULL){
      assert(msg.sib1_stats.msg.len != 0);
 } 
