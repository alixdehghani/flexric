...
byte_array_t enb_conf_enc_action_def_plain(enb_conf_action_def_t const* action_def)
{
...
byte_array_t enb_conf_enc_ind_hdr_plain(enb_conf_ind_hdr_t const* ind_hdr)
{
  assert(ind_hdr != NULL);

  byte_array_t ba = {0};

  ba.len = sizeof(enb_conf_ind_hdr_t);
...
byte_array_t enb_conf_enc_ind_msg_plain(enb_conf_ind_msg_t const* ind_msg)
{
  assert(ind_msg != NULL);
  ...
  size_t const sz = sizeof(ind_msg->len) + 
                  sizeof(enb_conf_stats_t)*ind_msg->len + 
...
byte_array_t enb_conf_enc_call_proc_id_plain(enb_conf_call_proc_id_t const* call_proc_id)
{
  assert(0!=0 && "Not implemented");
...
byte_array_t enb_conf_enc_ctrl_hdr_plain(enb_conf_ctrl_hdr_t const* ctrl_hdr)
{
  assert(ctrl_hdr != NULL);
  byte_array_t  ba = {0};
  ba.len = sizeof(enb_conf_ctrl_hdr_t);
  ba.buf = calloc(ba.len, sizeof(uint8_t)); 
  assert(ba.buf != NULL && "Memory exhausted");
...
byte_array_t enb_conf_enc_ctrl_msg_plain(enb_conf_ctrl_msg_t const* ctrl_msg)
{
  assert(ctrl_msg != NULL);

  byte_array_t  ba = {0};
  size_t const sz = sizeof(ctrl_msg->len) + 
                  sizeof(enb_conf_stats_t)*ctrl_msg->len;
  ...
  for(uint32_t i = 0; i < ctrl_msg->len ; ++i){
    memcpy(it, &ctrl_msg->enb_rb[i], sizeof(ctrl_msg->enb_rb[i]));
    it += sizeof(ctrl_msg->enb_rb[i]);
  }
...
byte_array_t enb_conf_enc_ctrl_out_plain(enb_conf_ctrl_out_t const* ctrl) 
{
  assert(0!=0 && "Not implemented");
...
byte_array_t enb_conf_enc_func_def_plain(enb_conf_func_def_t const* func)
{
  assert(0!=0 && "Not implemented");
