...
#include "../../enb_conf_sm/enb_conf_sm_agent.h"
#include "../../enb_conf_sm/enb_conf_sm_ric.h"
...
{
  assert(read != NULL);
  assert(read->type == ENB_CONF_STATS_V0);

  fill_enb_conf_ind_data(&read->enb_conf_stats);
  cp.hdr = cp_enb_conf_ind_hdr(&read->enb_conf_stats.hdr);
  cp.msg = cp_enb_conf_ind_msg(&read->enb_conf_stats.msg);
}
...
void check_indication(sm_agent_t* ag, sm_ric_t* ric)
{
 ...
 sm_ag_if_rd_t msg = ric->proc.on_indication(ric, &sm_data);
  ...
  enb_conf_ind_data_t* data = &msg.enb_conf_stats;

 if(msg.enb_conf_stats.msg.rb != NULL){
      assert(msg.enb_conf_stats.msg.len != 0);
 } 
