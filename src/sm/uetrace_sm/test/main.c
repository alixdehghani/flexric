...
#include "../../uetrace_sm/uetrace_sm_agent.h"
#include "../../uetrace_sm/uetrace_sm_ric.h"
...
{
  assert(read != NULL);
  assert(read->type == UETRACE_STATS_V0);

  fill_uetrace_ind_data(&read->uetrace_stats);
  cp.hdr = cp_uetrace_ind_hdr(&read->uetrace_stats.hdr);
  cp.msg = cp_uetrace_ind_msg(&read->uetrace_stats.msg);
}
...
void check_indication(sm_agent_t* ag, sm_ric_t* ric)
{
 ...
 sm_ag_if_rd_t msg = ric->proc.on_indication(ric, &sm_data);
  ...
  uetrace_ind_data_t* data = &msg.uetrace_stats;

 if(msg.uetrace_stats.msg.rb != NULL){
      assert(msg.uetrace_stats.msg.len != 0);
 } 
