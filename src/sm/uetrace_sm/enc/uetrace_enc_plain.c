...
byte_array_t uetrace_enc_event_trigger_plain(uetrace_event_trigger_t const* event_trigger)
{
...
byte_array_t uetrace_enc_action_def_plain(uetrace_action_def_t const* action_def)
{
...
byte_array_t uetrace_enc_ind_hdr_plain(uetrace_ind_hdr_t const* ind_hdr)
{
  assert(ind_hdr != NULL);

  byte_array_t ba = {0};

  ba.len = sizeof(uetrace_ind_hdr_t);
...
byte_array_t uetrace_enc_ind_msg_plain(uetrace_ind_msg_t const* ind_msg)
{
  assert(ind_msg != NULL);
  ...
  size_t const sz = sizeof(ind_msg->len) + 
                  sizeof(uetrace_stats_t)*ind_msg->len + 
...
byte_array_t uetrace_enc_call_proc_id_plain(uetrace_call_proc_id_t const* call_proc_id)
{
  assert(0!=0 && "Not implemented");
...
byte_array_t uetrace_enc_ctrl_hdr_plain(uetrace_ctrl_hdr_t const* ctrl_hdr)
{
  assert(ctrl_hdr != NULL);
  byte_array_t  ba = {0};
  ba.len = sizeof(uetrace_ctrl_hdr_t);
  ba.buf = calloc(ba.len, sizeof(uint8_t)); 
  assert(ba.buf != NULL && "Memory exhausted");
...
byte_array_t uetrace_enc_ctrl_msg_plain(uetrace_ctrl_msg_t const* ctrl_msg)
{
...
byte_array_t uetrace_enc_ctrl_out_plain(uetrace_ctrl_out_t const* ctrl) 
{
  assert(0!=0 && "Not implemented");
...
byte_array_t uetrace_enc_func_def_plain(uetrace_func_def_t const* func)
{
  assert(0!=0 && "Not implemented");
