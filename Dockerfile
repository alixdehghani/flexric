...
COPY --from=oai-flexric-builder \
    /usr/local/lib/flexric/libcounters_sm.so \
    /usr/local/lib/flexric/libenb_conf_sm.so \
...
COPY --from=oai-flexric-builder \
    /flexric/flexric.conf /usr/local/flexric/flexric.conf

## Copy xApps
COPY --from=oai-flexric-builder \
    /flexric/build/examples/xApp /usr/local/flexric/xApp

## Copy xApp configuration files
COPY --from=oai-flexric-builder \
    /flexric/ric.conf /usr/local/flexric/ric.conf

## copy emalulator
COPY --from=oai-flexric-builder \
    /flexric/build/examples/emulator/agent/emu_agent_enb /usr/local/bin/emu_agent_enb

## copy emalulator configuration files
COPY --from=oai-flexric-builder \
    /flexric/agent.conf /usr/local/flexric/agent.conf
...
CMD ["stdbuf", "-o0", "nearRT-RIC", "-c", "/usr/local/flexric/ric.conf"]
