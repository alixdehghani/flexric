...
#define ENB_ADDR_CONF_SEC_1 "https://192.168.80.222/bk/sector-1/configuration/enb"
#define ENB_ADDR_CONF_SEC_2 "https://192.168.80.222/bk/sector-2/configuration/enb"
#define ENB_SEC_NUM "https://192.168.80.222/bk/bts-sector-info"

void init_enb_conf_sm(void)
{
  printf("=========================1\n\n");
  client_enb = http_client_init();
  if (!client_enb)
  {
  ...
  http_result_t get_sec_num = http_get_custome(client_enb, ENB_SEC_NUM);
  ...
  json_object *sectors_array = json_get_array(json_parse_sec_num, "sectors");
    ...
    if (sectors_array != NULL) {
        ...
        int num_sectors = json_object_array_length(sectors_array);
        sector_num = num_sectors;
...
bool read_enb_conf_sm(void* data)
{
  ...
  http_result_t result_get = http_get_custome(client_enb, ENB_ADDR_CONF_SEC_1);
