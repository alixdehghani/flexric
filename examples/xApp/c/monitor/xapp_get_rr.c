 ...
 */
 ...
 static uint64_t cnt_rr;
 
  
 
 // RR !
 static void sm_cb_rr(sm_ag_if_rd_t const *rd)
 {
   assert(rd != NULL);
   ...
   if (cnt_rr % 1024 == 0)
     printf("rr ind_msg latency = %ld μs\n", now - rd->ind.rr.msg.tstamp);
 ...
 int main(int argc, char *argv[])
 {
   ...
   // rr indication
   const char *i_2 = "1_ms";
   sm_ans_xapp_t *rr_handle = NULL;
   
   if (nodes.len > 0)
   {
     rr_handle = calloc(nodes.len, sizeof(sm_ans_xapp_t));
     assert(rr_handle != NULL);
   }
   ...
   for (int i = 0; i < nodes.len; i++)
   {
     ...
     if (n->id.type == ngran_gNB || n->id.type == ngran_eNB)
     {
       ...
       rr_handle[i] = report_sm_xapp_api(&nodes.n[i].id, 156, (void *)i_2, sm_cb_rr);
       assert(rr_handle[i].success == true);
   ...
   for (int i = 0; i < nodes.len; ++i)
   {
     ...
     if (rr_handle[i].u.handle != 0)
       rm_report_sm_xapp_api(rr_handle[i].u.handle);
